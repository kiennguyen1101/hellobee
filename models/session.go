...
type Session struct {
	...
	SessionData   null.Bytes `boil:"session_data" json:"session_data,omitempty" toml:"session_data" yaml:"session_data,omitempty"`
	SessionExpiry uint       `boil:"session_expiry" json:"session_expiry" toml:"session_expiry" yaml:"session_expiry"`
...
var (
	sessionColumns               = []string{"session_key", "session_data", "session_expiry"}
	sessionColumnsWithoutDefault = []string{"session_key", "session_data", "session_expiry"}
	sessionColumnsWithDefault    = []string{}
	sessionPrimaryKeyColumns     = []string{"session_key"}
)
...
func FindSession(ctx context.Context, exec boil.ContextExecutor, sessionKey string, selectCols ...string) (*Session, error) {
	sessionObj := &Session{}
	...
	query := fmt.Sprintf(
		"select %s from `session` where `session_key`=?", sel,
	)
...
func (o *Session) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM `session` WHERE `session_key`=?"
...
func (o *Session) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindSession(ctx, exec, o.SessionKey)
	if err != nil {
		return err
	}
...
func SessionExists(ctx context.Context, exec boil.ContextExecutor, sessionKey string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `session` where `session_key`=? limit 1)"
