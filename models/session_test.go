...
func testSessionsExists(t *testing.T) {
	...
	e, err := SessionExists(ctx, tx, o.SessionKey)
	if err != nil {
...
func testSessionsFind(t *testing.T) {
	...
	sessionFound, err := FindSession(ctx, tx, o.SessionKey)
	if err != nil {
		t.Error(err)
	}
...
var (
	sessionDBTypes = map[string]string{`SessionData`: `blob`, `SessionExpiry`: `int`, `SessionKey`: `char`}
	_              = bytes.MinRead
)
